...
const { PAT_TOKEN: PAT, GITHUB_USERNAME: USERNAME } = process.env;
...
async function getCommitCountForRepo(owner, repoName) {
  try {
    ...
    if (!Array.isArray(stats.data)) {
      console.warn(`Stats for repo "${repoName}" are not ready or empty.`);
      return 0;
    }
...
async function getStats() {
  ...
  // Fetch commits counts sequentially to avoid hitting rate limits too fast
  let totalCommits = 0;
  ...
  // Fetch PR counts via search API
  ...
  // Fetch comments concurrently for performance
  const commentsPromises = repos.map(async repo => {
    ...
    const userIssueComments = issueComments.filter(c => c.user.login === USERNAME).length;
    const userPRComments = prReviewComments.filter(c => c.user.login === USERNAME).length;
    return userIssueComments + userPRComments;
  });

  const commentsArray = await Promise.all(commentsPromises);
  const totalComments = commentsArray.reduce((a, b) => a + b, 0);
